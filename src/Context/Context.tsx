...
interface BasketContextType {
  ...
  addToBasket: (product: ProductsType, quantity: number, color: string) => void;
...
export const BasketProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addToBasket = (product: ProductsType, quantity: number = 1, color: string) => {
    setBasketItems(prevItems => {
      // Check if the product already exists in the basket
      const existingItemIndex = prevItems.findIndex(item => item.id === product.id && item.color === color);
      ...
      if (existingItemIndex >= 0) {
        ...
        updatedItems = [...prevItems, { ...product, quantity,color }];
      }
